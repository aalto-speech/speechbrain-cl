...
from cl.utils.process_utils import normalize_text
from cl.utils.process_utils import strip_spaces
from cl.vad import testset_pipeline_with_force_segments
...
def fit(hparams, run_opts, overrides, ASR_Model=ASR):
    ...
    if hparams["sorting"] not in CurriculumDataset.CURRICULUM_KEYS:
        ...
        sorting_dict_log = os.path.join(
            _save_folder,
            f"{asr_brain.dict_log_prefix}{hparams['sorting']}_dict-epoch=0.log",
        )
        ...
        if os.path.isfile(sorting_dict_log):
            ...
            if len(asr_brain.sorting_dict) < len(asr_brain.train_set):
                logger.error(
                    strip_spaces(
                        "Could not load sorting dictionary since it is not full. \
    ...
    if do_fixed_sort_flag:
        logger.warning(
            strip_spaces(
                f"Will use a pretrained model to initialize the dictionary. \
    ...
    if hparams.get("use_fixed_sorting", False) and hparams.get(
        "pretrained_model_hparams", None
    ) not in [None, False]:
...
def dataio_prepare(hparams, device):
    ...
    if hparams["sorting"] in train_data.CURRICULUM_KEYS:
        ...
        hparams["subsampling_percentage"] = 1.0
    ...
    if hparams.get("use_vad", False):
        max_dur = hparams.get("max_duration_secs", 50.0)
        min_dur = hparams.get("min_duration_secs", 1.0)
...
def webdataio_prepare(hparams):
    ...
    if os.path.isfile(hparams.get("lp_filelist", "")):
        ...
        tok_kwargs["remove_special_tokens"] = hparams.get(
...
def get_tokenizer(hparams, device, annotation_read="wrd"):
    ...
    tok_kwargs = {}
    if (
        hparams.get("use_wav2vec2", False)
        or hparams["bos_index"] != hparams["eos_index"]
    ):
        tok_kwargs["bos_id"] = hparams["bos_index"]
        tok_kwargs["eos_id"] = hparams["eos_index"]
...
def _use_lm(hparams, device):
    if hparams.get("use_lm", False):
        if (
            "pretrainer" not in hparams
            or "tokenizer" not in hparams
            or "pretrained_lm_tokenizer_path" not in hparams
        ):  # This is probably not needed
            raise KeyError(
                strip_spaces(
                    "You should provide 'pretrainer', 'tokenizer' \
