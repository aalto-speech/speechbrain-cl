...
class CurriculumBase(DynamicItemDataset, ABC):
    ...
    def adaptive_pacing(
        ...
        if not isinstance(sorting_dict, dict) or len(sorting_dict) == 0:
            raise ValueError(
                f"Invalid sorting dictionary of type: {type(sorting_dict)}."
            )
        if normalize:
        ...
        logger.info(
            f'Saved paced indices under {os.path.join(tmp_path, "paced_sorted_ids.txt")}'
        )
        ...
        if getattr(self, "adaptive_pacing_index", None) is None:
            ...
            logger.warning(
                strip_spaces(
                    f"The adaptive pacing index has reached the maximum number \
        ...
        logger.info(
            f"Number of training samples in the current group: {len(current_indices)}"
        )
        ...
        if (
            self.adaptive_pacing_index[1] >= epochs_per_group
            and self.adaptive_pacing_index[0] < len(paced_sorted_ids) - 1
        ):
            self.adaptive_pacing_index[0] += 1
            self.adaptive_pacing_index[1] = 0
        self.adaptive_pacing_index[0] = min(
            self.adaptive_pacing_index[0], len(paced_sorted_ids) - 1
        )
...
class CurriculumDataset(CurriculumBase):
    ...
    def filtered_sorted(
        ...
        if sort_key not in (self.CURRICULUM_KEYS + ["random"]) or (not sorting_dict):
            ...
            raise ValueError(
                "You provided a curriculum sorting key but no losses. Aborting..."
            )
        else:
        ...
        if (
            isinstance(noise_percentage, float)
            and 0.0 < noise_percentage <= 1.0
            and sort_key != "duration"
        ):
            # logger.info(f"{filtered_sorted_ids[:10]=}")
            filtered_sorted_ids = CurriculumDataset.add_random_noise(
                filtered_sorted_ids, noise_percentage
            )
            logger.info("Added some random noise among the easy examples.")
            # logger.info(f"{filtered_sorted_ids[:10]=}")
    ...
    def normalize_dict(
        ...
        select_n = select_n or (
            getattr(self, "current_epoch_n_datapoints", None) or len(sorting_dict)
        )
        ...
        if isinstance(sd0, tuple) and self.sorting in self.METRIC_SORTERS:
            ...
            self.sorting in self.LOSS_SORTERS
            and isinstance(sd0, tuple)
            and len(sd0) == 2
        ):
    ...
    def _curriculum_filtered_ids(
        self,
        sorting_dict: Dict[
            str, Union[float, Tuple[float, float], Tuple[float, float, float]]
        ],
        reverse: bool = False,
        select_n: Optional[int] = None,
        ...
        sorting_dict = self.normalize_dict(
            sorting_dict, select_n=select_n, epsilon=epsilon
        )
        filtered_sorted_ids: list = [
            i
            for i in sorted(
                sorting_dict, key=lambda x: sorting_dict[x], reverse=reverse
            )
        ...
        if debug:
            ...
            for i, j in zip(filtered_sorted_ids[:-1], filtered_sorted_ids[1:]):
                if reverse:
                    assert (
                        sorting_dict[i] >= sorting_dict[j]
                    ), f"i:{i}, j:{j}, di: {sorting_dict[i]}, dj: {sorting_dict[j]}"
                else:
                    assert (
                        sorting_dict[i] <= sorting_dict[j]
                    ), f"i:{i}, j:{j}, di: {sorting_dict[i]}, dj: {sorting_dict[j]}"
    ...
    def _on_curriculum_end(
        self, brain: sb.core.Brain, original_keys: dict, sorting_dict: dict = None
    ):
...
class CurriculumSubset(CurriculumDataset):
    ...
    def __init__(
        ...
        self.data_ids = [
            data_id for idx, data_id in enumerate(self.data.keys()) if idx in indices
        ]
