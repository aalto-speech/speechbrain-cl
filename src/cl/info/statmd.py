...
try:
    from cl.info.globals import AVAILABLE_METRICS
    from cl.info.globals import DEFAULT_METRICS
except ImportError:
    DEFAULT_METRICS = ["WER", "CER"]
    AVAILABLE_METRICS = ["WER", "CER", "PER"]


class NoEpochsTrained(Exception):
    pass
...
def _read_stats(paths, metrics=DEFAULT_METRICS, return_test_results=False):
    ...
    for p in paths:
        if not os.path.isfile(p):
            raise FileNotFoundError(f"Invalid path to 'log.txt': {p}.")
    contents = []
    test_results = []
    for path in paths:
        with open(path) as f:
            content = f.readlines()
            test_results += [
                _check_test_stats(c, metrics) for c in content if "test loss:" in c
            ]
    ...
    def get_epoch_numbers():
        return [
            [
                c.split("epoch:")[1].split(",")[0].strip()
                for content in content_tuple
                for c in content
                if "epoch:" in c
            ]
            for content_tuple in zip(contents)
        ]
    ...
    try:
        ...
        raise NoEpochsTrained(
            "A provided path did not include a log with >= 1 epochs trained."
        )
    ...
    if any(
        len(epoch) != max_epoch_num for epoch, max_epoch_num in zip(epochs, max_epochs)
    ):
    ...
    if len(epochs) == 0:
        raise ValueError(
            "Something went wrong. Are you sure you have provided a valid log.txt file?"
        )
    # train_losses = [float(c.split("train loss:")[1].split(" - ")[0].strip()) for c in content]
    train_losses = [
        [
            float(c.split("train loss:")[1].split(" - ")[0].strip())
            for content in content_tuple
            for c in content
            if "epoch:" in c
        ]
    ...
    valid_losses = [
        [
            float(c.split("valid loss:")[1].split(",")[0].strip())
            for content in content_tuple
            for c in content
            if "epoch:" in c
        ]
    ...
    for metric in metrics:
        try:
            valid_metrics = [
                [
                    float(c.split(f"valid {metric}:")[1].split(",")[0].strip())
                    for content in content_tuple
                    for c in content
                    if "epoch:" in c
                ]
            ...
            raise ValueError(
                "Could not find a valid entry with the {} metric. Maybe you are using another metric?".format(
                    metric
                )
            )
    ...
    assert all(
        [
            len(epochs) == len(train_losses) == len(valid_losses) == len(valid_metric)
            for valid_metric in valid_metrics_dict.values()
        ]
    )
    assert all(
        [
...
def _check_test_stats(test_result, metrics):
    ...
    for metric in metrics:
        try:
            test_metrics_dict[metric] = float(
                test_result.split(f"test {metric}:")[1].split(",")[0].strip()
            )
        except IndexError:
            raise ValueError(
                "Could not find a valid entry with the {} metric. Maybe you are using another metric?".format(
    ...
    for i, (epochs, tls, vls) in enumerate(zip(epochs, train_losses, valid_losses)):
        ...
        for j, (tl, vl) in enumerate(zip(tls, vls)):
            if show_losses:
                s += f" {tl}  | {vl}  |"
            for metric in metrics:
                s += f" {valid_metrics_dict[metric][i][j]}  |"
        s += "\n"
    if len(out) == 5:
        test_results = out[4]
        for i, res in enumerate(test_results):
            if len(res) > 0:
                s += f"Model {i}: test_loss={test_results[i][0]}, "
                for metric in metrics:
                    s += ", test_{}={}".format(
                        metric.lower(), test_results[i][1][metric]
                    )
                s += "\n"
...
def _add_parser_args(parser=None):
    ...
    parser.add_argument(
        "paths", metavar="PATHS", nargs="+", help="Path(s) to log.txt files."
    )
    parser.add_argument(
        "--metrics",
        "-m",
        choices=AVAILABLE_METRICS + list(_metric_maps.keys()),
        default=[],
        nargs="*",
