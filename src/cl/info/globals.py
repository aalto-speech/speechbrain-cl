...
DARK_COLORS = ["black", "darkgray", "gray", "dimgray", "lightgray"]
MPL_COLORS = [
    "black",  # "pink",
    "brown",
    "gray",
...
STRATEGIES = ["WER", "SEQ", "WRD", "CHR", "TOK"]
...
def map_name_thesis(basename, remove_runs=True):
    ...
    basename = (
        ...
        .replace("ASCENDING", "DUR")
        .replace("-FULLASC", "")
        .replace("DESCENDING", "DUR$\\downarrow$")
        .replace("-BASE", "")
        .replace("AP", "VPF")
        .replace("RANDOM", "RND")
        .replace("TOKEN", "TOK")
...
def map_name(model_id, name_mappings):
    ...
    for subset_name in name_mappings["subset_mappings"]:
        if subset_name in model_id:
            name = "".join(
                ["(", name_mappings["subset_mappings"][subset_name], ")", " ", name]
            )
            break
