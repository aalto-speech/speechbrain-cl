...
def get_train_times(log_txt_pattern, silent=False):
    ...
    for path in iterator:
                ...
                if "Currently training for" in line:
                    ...
                    if epoch is None:
                        ...
                        if last_epoch_seen in minutes_per_epoch.keys():
                            minutes_per_epoch[last_epoch_seen] = max(
                                mins, minutes_per_epoch[last_epoch_seen]
                            )
                            continue
                    assert (
                        epoch is not None
                    ), "Model id: {}, Epoch: {}, mins: {}\nDict: {}".format(
                        identifier, epoch, mins, minutes_per_epoch
                    )
                    minutes_per_epoch[epoch] = mins
                    epoch, mins = None, None
                    continue
        if len(minutes_per_epoch) == 0:
            msg = f"Model {identifier} has not been trained yet."
            if silent:
                print(msg)
                continue
        ...
        if model_id in model_to_times:
            ...
            model_to_times[model_id] = dict(
                minutes_per_epoch=[minutes_per_epoch], seeds=[seed]
            )
        # model_to_times[identifier] = minutes_per_epoch
        print(
            "Model {} took: \t\t {} hours (epochs={})".format(
                identifier, total_time, len(set(minutes_per_epoch.keys()))
            )
        )
    out = {}
        ...
        if len(minutes_per_epoch) == 1:
            ...
            if len(set(n_epochs_per_run)) < len(n_epochs_per_run):
                ...
                for n_epochs, count in epoch_counts.items():
                    ...
                    common_indices = [
                        i
                        for i in range(len(n_epochs_per_run))
                        if len(minutes_per_epoch[i]) == n_epochs
                    ]
                    relevant = [
                        e
                        for i, e in enumerate(minutes_per_epoch)
                    ...
                    if count > 1:
                        ...
                        seed = [
                            ...
                            if len(minutes_per_epoch[i].keys()) == n_epochs
                        ][0]
                        identifier = f"{model_id} ({seed})"
...
def plot_train_times(log_txt_pattern, out_path=None, silent=False):
    ...
    for i, model_base in enumerate(model_to_times):
        try:
            ...
            relevant_entries = [
                v
                for k, v in model_to_best_val.items()
                if k.split("(")[0].strip() in model_base
            ]
        ...
        time_of_best_wer = [
            y for ind, y in enumerate(y_axis) if ind == best_epoch_index
        ][0]
        ...
        plot_data[model_base] = {
            ...
            "text": {"args": [best_epoch, time_of_best_wer, best_wer], "kwargs": {}},
            "plot2": {
...
def hours_to_wers_plot(paths: list, train_csv_name="train-complete_segmented.csv"):
    ...
    for directory in paths:
        ...
        if not os.path.isfile(log):
            print(
                f"Ignoring {directory} since it does not contain a train_log.txt file."
            )
            continue
        ...
        if identifier in hours:
            continue
        train_csv = os.path.join(directory, train_csv_name)
        with open(log) as fr:
            n_epochs = [
                int(l.split("epoch:")[1].split(",")[0].strip())
                for l in fr
        ...
        if is_paced:
            with open(os.path.join(directory, "hyperparams.yaml")) as fr:
                subsampling_n_epochs = [
                    l for l in fr if l.startswith("subsampling_n_epochs:")
                ]
            if len(subsampling_n_epochs) != 1:
                raise ValueError(
                    "Could not find the subsampling_n_epochs attribute even though a pacing function is used."
                )
            subsampling_n_epochs = int(
                subsampling_n_epochs[0].split()[-1].replace("\n", "").strip()
            )
...
def _find_best_epoch(epochs, valid_metrics_dict, metric="WER"):
    ...
    for i in range(1, len(valid_metrics_dict[metric])):
        if (
            valid_metrics_dict[metric][i][0]
            < valid_metrics_dict[metric][best_valid_index][0]
        ):
            best_valid_index = i
