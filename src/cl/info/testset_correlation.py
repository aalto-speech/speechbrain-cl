...
from cl.info.find_anomalies import FIRST_EXAMPLE_LINE
from cl.info.find_anomalies import SEPARATOR
...
def testset_corr(
    ...
    for i, wf in enumerate(wer_files):
        if not os.path.isfile(wf):
            ...
            continue
        with open(wf, encoding="utf-8") as fr:
            ...
            lines = {
                l.split(",")[0].strip(): float(l.split("[")[0].split("WER")[1].strip())
                for l in lines
            }
        ...
        model_name = (
            map_name(identifier, name_mappings).replace("CL: ", "")
            + identifier.split()[-2]
        )
        df_dict = pd.DataFrame(
            {"utterance_id": list(lines.keys()), model_name: list(lines.values())}
        )
    ...
    assert all(
        len(df) == len(df_dict) for df_dict in df_dicts
    ), f"{df.head()}\n{len(df)}\n{len(df_dicts[0])=}"
    ...
    # Add Plot Title
    plt.title(
        f"{corr_method.capitalize()} Correlation Between Model Pairs", fontsize=22
    )
