...
from .find_anomalies import separate_test_entry


UTT_ID_CSV_NAME = (
    "ID"  # Column name of the utterance identifier in the (test) .csv file.
)
...
def utterance_map(res_path, csv_path):
    ...
    with open(res_path, encoding="utf-8") as fr:
...
def plot_score_to_dur(utt2stats, title=None, out="tmp.eps"):
    ...
    utt2stats = {
        utt_id: v for utt_id, v in sorted(utt2stats.items(), key=lambda v: v[1][1])
    }
    ...
    utt2stats_new = {
        utt_id: (v[0], len(v[-1].split()))
        for utt_id, v in sorted(utt2stats.items(), key=lambda v: len(v[1][-1].split()))
    }
