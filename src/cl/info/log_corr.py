    ...
    for p in paths:
        if os.path.isdir(p):
            ...
            try:
                ...
                print(
                    f"Error while processing path {p} and current_paths being {current_paths}"
                )
                raise e
            show_later = True
        else:
...
def _calc_corr_single(*paths, **kwargs):
    ...
    for p in pbar:
        if "random" in p:
            raise Exception(
                "Random methods are not supported yet because the \
        ...
        if ("no_ga" in p) and ("tr" not in p):
            raise Exception(
                "for no curriculum you need to read the train csv file\
    ...
    corr_per_epoch = {}
    pbar = tqdm(
        zip(list(correlations.keys())[:-1], list(correlations.keys())[1:]),
        total=len(correlations) - 1,
    )
    ...
    if kwargs.pop("visualize", False) is True:
        plot_corr_per_epoch(
            corr_per_epoch, identifier, show=kwargs.pop("show", True), **kwargs
        )
...
def _get_parser(parser=None):
    ...
    parser.add_argument(
        "paths",
        nargs="+",
        help="Either a path to the directory \
