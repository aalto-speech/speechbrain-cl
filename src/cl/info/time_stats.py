    ...
    if is_paced:
        ...
        for i, f in tqdm(enumerate(shuffled_ids_paths)):
            ...
            data = [
                ...
                for idx, data_id in enumerate(dataset.data.keys())
            ...
            if i == 0:
                ...
                epochs_seen += min(
                    ...
                    max(1, n_epochs - ((i + 1) * subsampling_n_epochs)),
                )
                hours *= min(
                    subsampling_n_epochs,
                    max(1, n_epochs - ((i + 1) * subsampling_n_epochs)),
        ...
        print(
            "Saw {} epochs that saw {} hours of data.".format(
                epochs_seen, total_hours_seen
            )
        )
    else:
