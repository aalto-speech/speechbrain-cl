...
from cl.info.globals import DARK_COLORS
from cl.info.globals import MAPPINGS
from cl.info.globals import MPL_COLORS
from cl.info.globals import MPL_MARKERS
from cl.info.globals import map_name
from cl.info.globals import map_name_thesis

from .find_anomalies import read_single
from .get_train_times import _find_best_epoch

# from numpy import mod
from .statmd import NoEpochsTrained
from .statmd import _read_args
from .statmd import _read_stats
from .statmd import get_args
...
def plot_valid_results(
    ...
    for path in paths:
        ...
        if model_id in stats:
            ...
            stats[model_id] = {
                "epochs": [epochs],
                "valid_metrics_dicts": [valid_metrics_dict],
                "seeds": [seed],
            }
    for model_id, d in stats.items():
        ...
        if n_curr_models == 1:
            ...
            model_to_stats[identifier] = {
                "epochs": d["epochs"][0],
                "valid_metrics_dict": d["valid_metrics_dicts"][0],
            }
        else:
            ...
            if len(set(n_epochs_per_run)) < len(n_epochs_per_run):
                ...
                for n_epochs, count in epoch_counts.items():
                    ...
                    vm_dict = [
                        vmd[metric]
                        for i, vmd in enumerate(d["valid_metrics_dicts"])
                        if len(d["epochs"][i]) == n_epochs
                    ]
                    vm_dict = {
                        metric: list(
                            map(
                                lambda y: sum(y) / len(y),
                                map(lambda x: x[0], zip(*vm_dict)),
                            )
                        )
                    }
                ...
                for i, s in enumerate(d["seeds"]):
                    ...
                    model_to_stats[identifier] = {
                        "epochs": d["epochs"][i],
                        "valid_metrics_dict": d["valid_metrics_dicts"][i],
                    }
    ...
    for i, identifier in enumerate(model_to_stats):
        ...
        best_valid_epoch, best_valid_wer = _find_best_epoch(
            epochs, valid_metrics_dict, metric=metric
        )
        # model_id = os.path.basename(os.path.dirname(os.path.dirname(path)))
        # seed = os.path.basename(os.path.dirname(path)).split("-")[0]
        # identifier = f"{model_id} ({seed})"
        ...
        best_epoch_index = [
            ind for ind, x in enumerate(x_axis) if int(x) == best_valid_epoch
        ][0]
        ...
        ax.plot(
            x_axis,
            vms,
            marker=random_markers[i],
            ms=4,
            linewidth=4,
            label=f"{identifier_latex}",
        ...
        plot_data[identifier] = {
            ...
            "text": {
                "args": [best_valid_epoch, vm_best_wer, f"{best_valid_wer}"],
                "kwargs": {},
            },
            "plot2": {
    ...
    if plot_axins:
        ...
        x1, x2, y1, y2 = (
            max_epoch - min(10, step),
            max_epoch + min(step / 2, 2),
            min_wer - 1,
            min_wer + 4,
        )
...
def plot_train_valid_loss(
    ...
    def sum_losses_list(d, d_losses_key, n_epochs):
        l_list = [
            tld
            for i, tld in enumerate(d[d_losses_key])
            if len(d["epochs"][i]) == n_epochs
        ]
    ...
    random_colors = (DARK_COLORS * (1 + (n_models * 2) // len(DARK_COLORS)))[
        : n_models * 2
    ]
    random_markers = (MPL_MARKERS * (1 + (n_models * 2) // len(MPL_MARKERS)))[
        : n_models * 2
    ]
    # fig = plt.figure(figsize=(16, 12))
    # fig, axes = plt.subplots(2, math.ceil(len(model_to_stats)/2), sharex=True, figsize=(5.5,7))
    fig, axes = plt.subplots(
        2, math.ceil(len(model_to_stats) / 2), sharey=True, figsize=(5, 6)
    )
    ...
    for i, identifier in enumerate(model_to_stats):
        ...
        if len(model_to_stats) == 1:
            ...
            current_ax = axes[
                i
            ]  # axes[i//2][i%2] if len(model_to_stats)>2 else axes[i]
...
def plot_logs(paths, metrics=["PER"], output_path=None, print_seed=False):
    ...
    assert len(epochs[0]) <= len(
        MPL_COLORS
    ), f"You will need to rotate the MPL_COLORS list. {epochs=}"
    assert (
        len(epochs)
        == len(train_losses)
        == len(valid_losses)
        == len(valid_metrics_dict[metrics[0]])
    ), f"{epochs=}\n{train_losses=}\n{valid_losses=}\n{valid_metrics_dict=}"
    if len(epochs[0]) > 1:
    ...
    for i in range(len(paths)):
        ...
        plt.plot(
            x_axis, tls, label="Train Loss", color=random_colors[0]
        )  # rotate here (with mod)
...
def _testset_boxplot_single(
    wer_file,
    model_name=None,
    max_allowed_score=250.0,
    metric="WER",
    ...
    for entry in wer_lines:
        n_ins, n_dels, n_subs = map(
            lambda x: min(float(x), max_allowed_score), entry[2:5]
        )
        ins.append(n_ins)
        dels.append(n_dels)
        subs.append(n_subs)
    ...
    print(
        f"| {wer_file.split('seq2seq/')[1]}\t|\t{wer}\t|\t{int(sum(ins))}\t|\t{int(sum(dels))}\t|\t{int(sum(subs))}\t|"
    )
...
def testset_boxplot_comparison(args):
    ...
    assert (
        len(wer_files) >= 1
    ), f"You need to provide at least one wer_test.txt file or directory ({wer_files=})."
    wer_changes = {}
    final_wer_files = []
    ...
    for i, wer_file in enumerate(final_wer_files):
        ...
        single_res = _testset_boxplot_single(
            wer_file, model_name, return_all_edits=return_all_edits
        )
        if return_all_edits:
...
def plot_logs_dispatcher(args):
    ...
    if args.plot_valid_results is True:
        ...
        return valid_scores_grouped_bp(
            paths, metrics[0], output_path, args.model_name_mappings
        )
