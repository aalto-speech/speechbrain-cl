...
DEFAULT_METRIC_THRESHOLD = (
    50.0  # i.e. we will keep all entries with a WER > 50.0 in order to analyze them
)
...
def read_single(path, threshold=DEFAULT_METRIC_THRESHOLD, print_stats=True):
    ...
    with open(path, encoding="utf-8") as fr:
        ...
        if print_stats:
            print("=" * 60)
            print(
                f"Number of utts with more than {threshold} {'WER' if is_wer else 'CER'}: {len(lines)}."
            )
            print(
                f"Insertions: {sum(e[2] for e in lines)}  |  Deletions: {sum(e[3] for e in lines)}  |  Substitutions: {sum(e[4] for e in lines)}"
            )
            try:
                random_insertions = random.sample([e[-1] for e in lines if e[2] > 4], 2)
                print(
                    "Some random utterances with more than 4 insertions:\n  "
...
def _get_parser(parser=None):
    ...
    parser.add_argument(
        "paths", nargs="*", help="Path or paths to wer_test*.txt files."
    )
    parser.add_argument(
        "--error-threshold",
        "-t",
    ...
    parser.add_argument(
        "--out-log-path", "-ol", required=False, default=None, help="Not implemented."
    )
    return parser
    ...
    if args.compare:
        ...
        if n_paths % 2 != 0:
            raise argparse.ArgumentTypeError(
                "Since you used --compare, you must provide pairs of paths."
            )
        ...
        if len(wer_paths) != len(cer_paths):
            raise argparse.ArgumentTypeError(
                "You must provide an equal number of wer_test*.txt and cer_test*.txt files."
            )
