...
from .info.find_anomalies import _get_parser as test_anomalies_parser
from .info.find_anomalies import _parse_args as test_anomalies
from .info.get_train_times import main as get_train_times
from .info.plot_metric_per_length import _get_parser as parse_test_wrt_durations
from .info.plot_metric_per_length import _parse_args as plot_test_per_length
from .info.read_wer_test import _get_parser as test_wer_parser
from .info.read_wer_test import read_wer_test
from .info.statmd import _add_parser_args
from .info.statmd import get_args
from .info.statmd import statmd

# from .info.log_corr import _get_parser as _get_corr_parser, _parse_args as _parse_corr_args
from .info.testset_correlation import _get_parser as _get_test_corr_parser
from .info.testset_correlation import _parse_args as _parse_corr_args
from .info.visualize import plot_logs_dispatcher
from .info.visualize import violinplots_by3 as testset_boxplot_comparison
from .utils.convert_txt_to_trn import _get_parser as convert_to_trn_parser
from .utils.convert_txt_to_trn import _parse_args as convert_to_trn
from .utils.kaldi_format_calc_wer import _get_parser as kaldi_calc_wer_parser
from .utils.kaldi_format_calc_wer import _parse_args as kaldi_calc_wer
from .utils.remove_repetitions import _get_parser as remove_reps_parser
from .utils.remove_repetitions import _parse_args as remove_reps
from .utils.sb_wer_to_kaldi import _get_parser as sb_wer_parser
from .utils.sb_wer_to_kaldi import _parse_args as sb_wer_to_kaldi
from .utils.significance_tests import _get_parser as get_sign_test_parser
...
def dispatch(predefined_args=None):
    ...
    test_and_dur_parser = subparsers.add_parser(
        "test_and_duration",
        aliases=["td"],
        help="Print wer scores on the test set w.r.t. the durations of the utterances.",
    )
    ...
    testwer_parser = subparsers.add_parser(
        "testwer",
        aliases=["tw"],
        help="Print wer scores on the test set with the ability to make a barplot.",
    )
    ...
    statmd_parser = subparsers.add_parser(
        "statmd",
        aliases=["s", "st"],
        help="Prints a list of train/dev losses and metric\
    ...
    def statmd_func(args):
        paths, metrics, output_path, no_show_losses = get_args(
            statmd_parser, args=args, add_args=False
        )
    ...
    log_plot_parser = subparsers.add_parser(
        "logplot",
        aliases=["l"],
        help="Produces some plots from the log.txt\
    ...
    train_times_parser = subparsers.add_parser(
        "traintimes",
        aliases=["t", "times"],
        help="Simple line plots\
            denoting the training times (for each epoch) that each \
            model requires",
    )
    train_times_parser.add_argument(
        "input",
        nargs="*",
    ...
    train_times_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If provided, we will also plot the train \
    ...
    train_times_parser.add_argument(
        ...
        required=False,
        default=None,
        help="If provided, the output plot (assuming -v is \
    ...
    train_times_parser.add_argument(
        "--show-hours-per-model",
        "--hpm",
        dest="show_hours_per_model",
        default=False,
        action="store_true",
    ...
    corr_parser = subparsers.add_parser(
        "correlations",
        aliases=["c"],
        help="Calculate correlations model pairs.\
    ...
    conv_parser = subparsers.add_parser(
        "convert",
        aliases=["cv"],
        help="Convert log files to specified formats.\
    ...
    if predefined_args is not None:
        return parser.parse_args(predefined_args)
    if len(sys.argv) == 1:
