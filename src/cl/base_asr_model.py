...
from cl.curriculum import CurriculumBase
from cl.curriculum import CurriculumDataset
from cl.curriculum import CurriculumSubset
from cl.methods.frequency_cl import FilteredSortedFrequencyCL
from cl.methods.frequency_cl import FrequencyCL
from cl.utils.process_utils import checkpoint_wrapper_cl
from cl.utils.process_utils import load_sorting_dictionary
from cl.utils.process_utils import save_sorting_dictionary
...
YamlTupleSafeLoader.add_constructor(
    "tag:yaml.org,2002:python/tuple", YamlTupleSafeLoader.construct_python_tuple
)
...
class BaseASR(sb.core.Brain, ABC):
    ...
    VALID_CL_KEYS = (
        ...
        + CurriculumDataset.CURRICULUM_KEYS
        + FrequencyCL.VALID_FREQUENCY_TYPES
    )
    ...
    def __init__(
        ...
        if self.train_set is None:
            raise AttributeError(
                "`train_set` argument not passed. You should have passed a valid speechbrain dataset instead."
            )
        self._curriculum_log_saved = (
            False  # dummy flag to make sure we don't overwrite the logs
        )
        ...
        self.current_training_time = 0.0
        ...
        self.sorting_dict: dict = (
            sorting_dict or final_sorting_dict or {}
        )  # initialize it and it will be filled while training
        ...
        self._loaded_checkpoint = (
            False  # used in utils.py (checkpoint_wrapper_cl function)
        )
        ...
        self.use_fixed_sorting = (
            getattr(self.hparams, "use_fixed_sorting", False) is True
        ) and hasattr(self.hparams, "pretrained_model_hparams")
        ...
        self.sorting_dict_needs_save = (
            (self.sorting != "random")
            and (not self.use_fixed_sorting)
            or self.do_subsample
        )
        if self.use_fixed_sorting and len(self.final_sortings) == 0:
            try:
                self.final_sortings = self.load_sorting_dict(epoch=0, inplace=False)
                logger.info(
                    f"Loaded dictionary with final sortings. Length: {len(self.final_sortings)}."
                )
    ...
    def make_random_dataloader(self):
        ...
        random_ids_path = os.path.join(self.hparams.output_folder, f"random_ids.npy")
        ...
        dataloader = self.make_dataloader(
            dataset=self.train_set, stage=sb.Stage.TRAIN, **self.train_loader_kwargs
        )
    ...
    def on_stage_start(self, stage, epoch):
        ...
        # ############################################################
        if self.do_adaptive_pacing and len(self.sorting_dict) < len(
            self.current_trainset
        ):
            logger.warning(
                f"Using an adaptive pacing function is not possible\
        ...
        if self.do_adaptive_pacing and len(self.sorting_dict) == len(
            ...
            logger.info(
                f"Using and adaptive pacing function on a sorting dictionary of length: {len(self.sorting_dict)}."
            )
        ...
        # ############################################################
        if (
            (self.current_epoch % self.curriculum_update_every) > 0
            and (self.sorting in CurriculumDataset.CURRICULUM_KEYS)
        ...
        if self.do_subsample and self.sorting != "random":
            ...
            increase_type = getattr(
                self.hparams, "subsampling_increase_type", "additive"
            )
            step_length = getattr(self.hparams, "subsampling_step_length", 5)
            logger.info(
                f"On stage start: Before subsample. Type of train_set: {type(self.train_set)}."
            )
            ...
            logger.info(
                f"On stage start: After subsample. Type of train_set: {type(self.train_set)}."
            )
        ...
        def load_and_sort():
            ...
            sb.dataio.dataset.set_output_keys(
                [self.train_set],
                self.train_set.pipeline.output_mapping,
            )
            reverse = getattr(self.hparams, "reverse", False)
            if len(self.final_sortings) == 0:
                raise Exception(
                    "You should have already loaded a checkpoint at this point."
                )
            if self.use_fixed_sorting:
                assert len(self.final_sortings) >= len(
                    self.train_set
                ), f"{len(self.final_sortings)=} --- {len(self.train_set)=}"
            ...
            if len(self.final_sortings) < len(self.train_set):
                raise Exception(
                    f"This can't happen: {len(self.final_sortings)=} === {len(self.sorting_dict)=} === {len(self.train_set)=}."
                )
        ...
        if self.use_default_training:
            logger.info(
                f"Epoch {epoch}: Sorting the dataset based on the duration of the examples before the first epoch."
            )
            return default_sort()
        ...
        if self.sorting in getattr(self.train_set, "LOSS_SORTERS", []):
            if len(self.sorting_dict) > 0:
                logger.debug(
                    f"Curriculum Learning: Using the sorting dictionary based on the {self.sorting} method."
                )
            ...
            logger.warning(
                "Default sorting, even though the sorting method is: {}.".format(
                    self.sorting
                )
            )
            return default_sort()
        ...
        if (
            isinstance(getattr(self.train_set, "sorting", None), tuple)
            and not getattr(self.train_set, "sorting", [""])[0]
            in self.train_set.CURRICULUM_KEYS
        ):
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            logger.info(
                f"Currently training for {round(self.current_training_time/60, 2)} minutes."
            )
    ...
    def adaptive_pacing(
        ...
        if not isinstance(self.train_set, CurriculumBase):
            raise NotImplementedError(
                f"Cannot use adaptive pacing with metadata-based workflow. {type(self.train_set)=}"
            )
        if len(self.sorting_dict) == 0:
            raise ValueError(
                "The length of the sorting dictionary cannot be 0 when using an adaptive pacing function."
            )
        ...
        try:
            ...
            logger.info(
                f"Other hyperparameters: {n_difficulty_groups=} -- {epochs_per_group=} -- {incremental=} -- {normalize=} -- {self.reverse=} -- {self.current_epoch=}."
            )
            if len(sd) > 0:
                key1 = list(sd.keys())[0]
    ...
    def subsample_trainset(
        ...
        if isinstance(increase_factor, float) and increase_factor > 0:
            if increase_type in ["additive", "+"]:
                percentage = round(
                    min(1.0, percentage + (increase_factor) * (dummy_epoch - 1)), 4
                )
                ...
                assert (
                    increase_factor > 1
                ), "Multiplicative should be above 1 since eitherwise the trainset will get shrinked to 0"
                epoch_to_use = (
                    max(update_every * (dummy_epoch - 1), 2) - 1
                )  # e.g. 1, 4, 9, 14...
                percentage = round(
                    min(
                        1, percentage * increase_factor ** (epoch_to_use / step_length)
                    ),
                    4,
                )
        ...
        def get_train_subset(shuffled_train_ids):
            if self.use_default_training:
                train_set = self.train_set.filtered_sorted(
                    sort_key="duration",
                    noise_percentage=getattr(
                        self.hparams, "noisy_random_percentage", None
                    ),
                )
            else:
                train_set = self.train_set
            if isinstance(train_set, FilteredSortedDynamicItemDataset) or isinstance(
                train_set, FilteredSortedFrequencyCL
            ):
                ...
                logger.info(
                    f"Using curriculum subset for filteredsorted dataset. {self.use_default_training=}"
                )
                ...
                # For random sorting
                raise NotImplementedError(
                    f"Cannot subsample a dataloader train set of type {type(self.train_set)} ({self.sorting=})."
                )
            elif isinstance(train_set, CurriculumDataset):
                train_subset = CurriculumSubset(train_set, shuffled_train_ids)
            else:
                raise TypeError(
                    f"Cannot subsample train set of type {type(train_set)} ({self.sorting=})."
                )
            return train_subset
        ...
        if percentage >= 1 and self.use_fixed_sorting:
            ...
            self.sorting_dict = self.load_sorting_dict(
                sorting_dict_log=curr_log_path, inplace=False
            )
            if (
                len(self.final_sortings) < len(self.sorting_dict)
                and self.use_fixed_sorting
            ):
                self.final_sortings = self.load_sorting_dict(epoch=0, inplace=False)
            ...
            logger.info(
                strip_spaces(
                    f"Loaded a precomputed train set with \
                {len(shuffled_train_ids)} datapoints (out of {len(self.train_set)})."
                )
            )
        else:
            shuffled_train_ids = np.random.permutation(len(self.train_set))[
                : round(percentage * len(self.train_set))
            ]
            self.train_subset = get_train_subset(shuffled_train_ids)
            assert hasattr(
                self.train_subset, "filtered_sorted"
            ), f"{type(self.train_subset)=}"
            if self.current_epoch == 1 and len(self.sorting_dict) > 0:
                logger.info(f"Selecting {len(shuffled_train_ids)} random ids.")
                self.sorting_dict = {
                    k: self.sorting_dict[k] for k in self.train_subset.data_ids
                }
            elif self.use_fixed_sorting and len(self.final_sortings) > len(
                self.sorting_dict
            ):
                logger.info(
                    f"Selecting {len(shuffled_train_ids)} random ids from the fixed sortings."
                )
                self.sorting_dict = {
                    k: self.final_sortings[k] for k in self.train_subset.data_ids
                }
            else:
                logger.info("We are going to create a new sorting dictionary.")
                if (
                    self.sorting in CurriculumDataset.CURRICULUM_KEYS
                ) and not self.use_default_training:
                    ...
                    logger.info(
                        f"Created a curriculum dictionary of size: {len(self.sorting_dict)=}."
                    )
                    if len(self.sorting_dict) > len(shuffled_train_ids):
                        logger.warning(
                            f"CREATED A CURRICULUM OF SIZE {len(self.sorting_dict)} WHILE THE SUBSET SHOULD BE OF LENGTH {len(shuffled_train_ids)}."
                        )
                        self.sorting_dict = {
                            k: v
                            for k, v in self.sorting_dict.items()
                            if k in self.train_subset.data_ids
                        }
            np.save(subset_ids_path, shuffled_train_ids)
            logger.info(
                strip_spaces(
                    f"Calculated a new train set with \
        ...
        if (self.sorting in BaseASR.DURATION_CL_KEYS) or self.use_default_training:
            logger.info(
                "Duration-based sorting + subsampling. This implies that the `noisy` CL method cannot be used."
            )
            train_set = self.train_subset.filtered_sorted(
                sort_key="duration",
                reverse=True if self.sorting == "descending" else False,
            )
        elif self.use_transfer_cl and isinstance(
            self.train_subset, FilteredSortedDynamicItemDataset
        ):
        ...
        logger.info(
            f"Size of the 'subsampled' trainset: {len(train_set)} and type: {type(train_set)} \
    ...
    def load_sorting_dict(
                ...
                try:
                    ...
                    epoch = max(
                        map(
                            int,
                            map(
                                lambda filename: filename.split("epoch=")[1].split(".")[
                                    0
                                ],
        ...
        assert os.path.isfile(
            sorting_dict_log
        ), f"Could not locate the presumed sorting_dict log file: {sorting_dict_log}"
    ...
    def create_curriculum_dict(
        ...
        if self.use_fixed_sorting:
            logger.info(
                f"Will process {len(train_set.dataset)} data points from transfer learning CL."
            )
        with tqdm(
            train_set,
            initial=self.step,
    ...
    def _save_curriculum_log(self, stage, epoch: int, sorting_dict: dict):
        if self.sorting not in self.VALID_CL_KEYS or len(self.sorting) == 0:
            return
        if (
            self._curriculum_log_saved is True
            or stage != sb.Stage.TRAIN
            or (not self.sorting_dict_needs_save)
        ):
            return
        ...
        def __save__(sorting_dict_log, ordered_examples):
            ...
            logger.info(f"Log file saved under: {sorting_dict_log}")
            self._curriculum_log_saved = True
        ...
        if self.sorting in CurriculumDataset.CURRICULUM_KEYS:
            ...
            isinstance(
                self.train_set,
                (FilteredSortedDynamicItemDataset, FilteredSortedFrequencyCL),
            )
            ...
            sorting_dict_log = os.path.join(
                self.hparams.output_folder,
                f"{self.dict_log_prefix}{self.sorting}_dict.log",
            )
    ...
    def _get_ordered_examples(self, sorting_dict):
        if self.sorting in CurriculumDataset.CURRICULUM_KEYS:
            ...
            ordered_examples = [
                f"{k}\t{v}"
                for k, v in sorted(
                    sorting_dict.items(), key=lambda x: x[1], reverse=True
                )
            ]
        elif (
            isinstance(
            ...
            if self.sorting in FrequencyCL.VALID_FREQUENCY_TYPES:
                ...
                ordered_examples = [
                    ...
                    for key, value in sorted(
                        ordered_dict.items(), key=lambda x: x[1], reverse=True
                    )
                ]
            else:
                ...
                ordered_examples = [
                    f"{tup[2]}\t{tup[0]}" for tup in sorted(filtered_ids, reverse=True)
                ]
        else:
            raise NotImplementedError(
                f"Could not save sorting dict for method {self.sorting}\
    ...
    def _metric_curriculum_update(
        self, stage, batch, loss, predictions, sorting_method=None
    ):
        ...
        if not hasattr(self, "tokenizer"):
            raise AttributeError(
                "Expected 'tokenizer' to exist for Metric-based Curriculum Learning."
        ...
        predicted_words = [
            re.sub(r"\.\w+", "", " ".join(txt)).split() for txt in predicted_words
        ]
        target_words = [
            re.sub(r"\.\w+", "", " ".join(txt)).split() for txt in target_words
        ]
    ...
    def _compute_seq_loss(
        ...
        if (
            (
                sorting_method is None
                and len(self.sorting_dict) < len(self.current_trainset)
            )
            or not self.do_adaptive_pacing
            or stage != sb.Stage.TRAIN
        ):
        ...
        if not self.do_adaptive_pacing:
            ...
            ), f"{sorting_method=}...{self.sorting=}...{len(self.sorting_dict)=}...{len(self.current_trainset)=}...{stage=}"
        ...
        if stage != sb.Stage.TRAIN:
            ...
            pass
        elif (self.current_epoch % self.curriculum_update_every != 0) and (
            not self.use_default_training
        ):
    ...
    def _compute_ctc_loss(
        self, predictions, batch, stage=None, reduction="mean", sorting_method=None
    ):
        # NOTE: You will probably have to override this method.
    ...
    def _save(self, path):
        try:
            self.current_training_time = (
                self.current_training_time + time.perf_counter() - self.start_epoch_time
            )
            save_dict = {
                "step": self.step,
    ...
    def _recover(self, path, end_of_epoch, device):
        ...
        if (
            self.sorting in CurriculumDataset.CURRICULUM_KEYS
            and self.use_fixed_sorting is True
        ):
            if len(self.sorting_dict) == 0:
                self.load_sorting_dict(epoch=0, inplace=True)
            return
