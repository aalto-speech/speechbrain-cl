...
class BaseCurriculum(ABC):
    ...
    def is_fully_loaded(self, train_set):
        if self.sorting_dict.get("num_datapoints") == len(train_set):
            logger.info(
                strip_spaces(
                    f"Seems like the curriculum dictionary is fully loaded. \
...
class MetricCurriculum(BaseCurriculum):
    def __init__(
        ...
        if self.sorting_method == "cer":
            ...
            raise Exception(
                "This sorting method has not been implemented yet:", self.sorting_method
            )
    ...
    def _process_batch(self, batch, avg, step=None):
        ...
        predicted_words: list = self.postprocess(
            self.brain.tokenizer(predicted_tokens, task="decode_from_list")
        )
        # Convert indices to words
        target_words = undo_padding(tokens, tokens_lens)
        target_words = self.postprocess(
            self.brain.tokenizer(target_words, task="decode_from_list")
        )
        ...
        # )
        loss_equivalent = torch.sum(
            torch.Tensor(
                [
                    details[dp_id]["WER"] / tokens_lens[dp_id]
...
class LossCurriculum(BaseCurriculum):
    def __init__(self, sorting_method, brain, sorting_dict=None, **kwargs):
        ...
        try:
            ...
            raise ValueError(
                f"""Invalid sorting method: {self.sorting_method}.
We could not locate a brain method with the name: _compute_{self.sorting_method}"""
    ...
    def _process_batch(self, batch, avg, step):
        ...
        # Get losses for each datapoint in the batch
        loss = self.method(
            predictions, batch, stage=sb.Stage.TRAIN, reduction="batch"
        ).detach()
        # 'id' must also be an output key
        # batch.id[0] since we only have one datapoint here.
        avg = self.update_average(
            new_val=self._mean_reduction(loss, batch), avg=avg, step=step
        )
        # self.n_samples += len(batch.id)
        for dp_id in range(len(batch.id)):
            self.sorting_dict[batch.id[dp_id]] = self.get_loss(
                round(loss[dp_id].item(), 4), batch.duration[dp_id].cpu().item()
            )
            logger.debug(
...
class JointCurriculum:
    def __init__(
        ...
        **kwargs,
        ...
        """
        self.loss_curriculum = LossCurriculum(
            loss_method, brain, sorting_dict, **kwargs
        )
        self.metric_curriculum = MetricCurriculum(
            metric_method, brain, sorting_dict, **kwargs
        )
        ...
        # TODO
        logger.error(
            f"""Loss curriculum and Metric curriculum output values in different ranges
...
range (after normalization)."""
    ...
    def sorting_dict(self):  # will also normalize the dataset
        ...
        assert len(self._inner_sort_dict) == (
            len(self.loss_curriculum.sorting_dict)
            + len(self.metric_curriculum.sorting_dict)
        )
    ...
    def _normalize(self):
        ...
        logger.info(
            "Normalizing loss and metric based curriculum dictionaries to the same range."
        )
    ...
    def is_fully_loaded(self, train_set):
        if self._inner_sort_dict.get(
            "num_datapoints", len(self._inner_sort_dict)
        ) == len(train_set):
            logger.info(
                strip_spaces(
                    f"Seems like the curriculum dictionary is fully loaded. \
