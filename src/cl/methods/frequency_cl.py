...
class InvalidFrequencyType(Exception):
    pass


class NoTokenizerError(Exception):
    pass
...
class FrequencyCL(CurriculumDataset):
        ...
        if self.frequency_type == "char":
            ...
            if self.tokenizer is None:
                raise NoTokenizerError(
                    f"The 'token' CL method requires you to provide a tokenizer."
                )
            assert isinstance(self.tokenizer, SentencePiece) or isinstance(
                self.tokenizer, FileListTokenizer
            ), "You should provide a valid tokenizer."
        else:
            raise InvalidFrequencyType(
                f"Invalid type of frequency calculator: {self.frequency_type}"
            )
    ...
    def filtered_sorted(
        ...
        if sort_key in self.VALID_FREQUENCY_TYPES:
            filtered_sorted_ids = self._curriculum_filtered_ids(
    ...
    def _curriculum_filtered_ids(
        ...
        select_n = select_n or (
            getattr(self, "current_epoch_n_datapoints", None) or len(self)
        )
        sorting_dict = sorting_dict or {}
        if len(sorting_dict) == len(self) and len(sorting_dict) >= len(
            self.precomputed_freqs
        ):
            self.precomputed_freqs = sorting_dict.copy()
            logger.warn(
                "Using the sorting dictionary as precomputed freqs. This may lead to errors when\
        ...
        filtered_sorted_ids: list = [
            utt_id
            for utt_id in sorted(
                reverse_freqs, key=lambda x: reverse_freqs[x], reverse=not reverse
            )
        ][:select_n]
    ...
    def _calculate_char_frequency(self):
        ...
        with self.output_keys_as(["wrd", "id"]) as dataset_trans:
            ...
            for item in dataset_trans:
                ...
                easyness = sum(self.precomputed_freqs[char] for char in trans) / len(
                    trans
                )
    ...
    def _calculate_token_frequency(self):
        ...
        reverse_sorting_dict = {}
        tokenize = (
            self.tokenizer.sp.encode_as_ids
            if hasattr(self.tokenizer, "sp")
            else self.tokenizer.encode_as_ids
        )
        with self.output_keys_as(["wrd", "id"]) as dataset_trans:
            if len(self.precomputed_freqs) != len(dataset_trans):
                tokens = []
