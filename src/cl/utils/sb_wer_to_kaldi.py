...
def convert_to_kaldi_text(
    ...
    if os.path.isfile(output_kaldi_text_path) and not overwrite_if_exists:
        warnings.warn(
            f"File {output_kaldi_text_path} already exists. We won't overwrite it."
        )
        return
    print(
        "Writing:",
        output_kaldi_text_path,
        "and ground truth:",
    ...
    with open(output_kaldi_text_path, "w") as fw:
        ...
        for utt_id, score, ins, dels, subs, truth, preds in contents:
            text = "{} {}\n".format(
                ...
                re.sub(r"\s+", " ", preds.strip().replace("\n", "")),
            )
            fw.write(text)
            if out_ground_truth_kaldi_path is not None:
                truth_text = "{} {}\n".format(
                    utt_id.strip().replace("\n", ""),
                    re.sub(r"\s+", " ", truth.strip().replace("\n", "")),
                )
...
def _get_parser(parser=None):
    ...
    parser.add_argument(
        "sb_test_paths", nargs="*", help="Path or paths to wer_test*.txt files."
    )
    parser.add_argument(
        "--out-kaldi-path",
        "-okp",
...
def _parse_args(args=None):
    ...
    if len(args.out_kaldi_path) == 0:
        kaldi_filename = lambda p: os.path.join(
            os.path.dirname(p), os.path.basename(p).split(".")[0] + ".kaldi"
        )
        args.out_kaldi_path = [kaldi_filename(p) for p in args.sb_test_paths]
    kaldi_truth_filename = lambda p: os.path.join(
        os.path.dirname(p), os.path.basename(p).split(".")[0] + "_truth.kaldi"
    )
    out_ground_truth_kaldi_path = [kaldi_truth_filename(p) for p in args.sb_test_paths]
    assert len(args.sb_test_paths) == len(
        args.out_kaldi_path
    ), f"{len(args.sb_test_paths)=}  {len(args.out_kaldi_path)=}"
    assert (
        len(args.sb_test_paths) > 0
    ), f"Length of input and output paths is {len(args.sb_test_paths)}."
    for in_path, out_path, truth_out_path in zip(
        args.sb_test_paths, args.out_kaldi_path, out_ground_truth_kaldi_path
    ):
        convert_to_kaldi_text(
            in_path, out_path, truth_out_path, DEFAULT_METRIC_THRESHOLD
        )
