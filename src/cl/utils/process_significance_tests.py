...
SIGNIFICANCE_TO_MAX_P = {"***": 0.001, "**": 0.01, "*": 0.05}
...
def _hyp_path_to_mname(path):
    return (
        ...
        .replace("_ga", "")
        .replace("noisy", "N")
        .replace("transfer_fixed", "trf")
        .replace("transfer", "TR")
        .replace("adaptive_pacing", "AP")
        .replace("seq_loss", "SEQ")
        .replace("_", "-")
        .upper()
        .replace("SUBSAMPLING", "SUB")
...
def process_unified_file(filename, metric="MP"):
    with open(filename) as f:
        contents = [
            re.sub(r"\s+", " ", l.strip().replace("\t", " ")) for l in f.readlines()
        ]
        mnames = [re.findall(r"\|\s+Test\s+\|\|", l) for l in contents]
        mnames = contents[[i for i in range(len(mnames)) if len(mnames[i]) > 0][0]]
        mnames = re.sub(r"\|\s+Test\s+\|\|", "", mnames)
        mnames = [
            m.strip()
            for m in mnames.split("|")
            if len(m.strip()) > 0 and m.strip() != "Test"
        ]
