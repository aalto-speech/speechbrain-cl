...
def kaldi_wer_from_files(pred_files, ground_truth_files, is_cer=False):
    ...
    for p, gt in pbar:
        ...
        save_file = os.path.join(
            os.path.dirname(p), os.path.basename(p).split(".")[0] + ".txt"
        )
...
def _parse_args(args=None):
    ...
    preds = [
        os.path.join(p, prefix + "_test_noreps.kaldi") for p in args.exps if finished(p)
    ]
    truths = [
        os.path.join(p, prefix + "_test_truth_noreps.kaldi")
        for p in args.exps
        if finished(p)
    ]
