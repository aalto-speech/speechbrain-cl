...
from .visualize import MPL_COLORS
from .statmd import _read_stats, NoEpochsTrained

def get_train_times(log_txt_pattern):
    model_to_times = {}
    ...
    for path in iterator:
        ...
        if len(minutes_per_epoch) == 0:
            raise NoEpochsTrained("Model {} has not been trained yet.".format(identifier))
...
def plot_train_times(log_txt_pattern, out_path=None):
    model_to_times = get_train_times(log_txt_pattern)
...
def _parse_args():
    ...
    parser.add_argument("input", help="E.g. './path/to/recipes/*/*/log.txt' or './path/to/log.txt'")
...
def main(args):
    if args.visualize:
        plot_train_times(args.input, args.out_plot_path)
    else:
        get_train_times(args.input)
